...
tag in `compose.pins.yml` while iterating) so that a rebuild upstream never
...
## Requirement: `compose.pins.yml` stays outside the image-build path filter

`compose.pins.yml` SHALL live at the repository root, outside `.devcontainer/`
...
`compose.pins.yml` overrides it with the digest-qualified reference via
