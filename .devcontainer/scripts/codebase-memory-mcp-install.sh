...
echo "codebase-memory-mcp install: user=$(id -un) home=$HOME cache_dir=$cache_dir binary=$cbm_bin_path"
echo "umask=$(umask)"
...
if ((install_status != 0)); then
  ...
  activation_log="$cache_dir/logs/activation-events.ndjson"
  ...
  daemon_log="$cache_dir/logs/cbm-daemon.log"
  ...
  conflicts_log="$cache_dir/logs/daemon-conflicts.ndjson"
